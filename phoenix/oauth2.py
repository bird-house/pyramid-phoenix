...
class OAuth2(object):
    ...
    def refresh_token(self, userid):
        ...
        try:
            ...
            token = client.refresh_token(self.refresh_url, client_id=self.client_id, client_secret=self.client_secret)
    ...
    def introspect_access_token(self, access_token):
        resp = requests.post(
            self.introspect_url, data={"token": access_token}, auth=(self.client_id, self.client_secret)
        )
        return resp.json()
