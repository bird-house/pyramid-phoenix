#!/usr/bin/env python
# FIXME: change above #! to use the "cli" function as an entry point in setup.py
# See also __main__ bit at end
...
def cli():
    ...
    parser.add_argument('--NAME', action='store_true',
                        help='equivalent to: -k process_id -r NAME')
    args = parser.parse_args()
    if args.NAME:
        args.key = 'process_id'
        args.regex = 'NAME'
    else:
        if args.key is None or args.regex is None:
            raise Exception("need to set key and regex")
    wsg = WPSStatsGetter()
    wsg.main(args.key, args.regex)


# FIXME: change this use the "cli" function as an entry point in setup.py
# see also shebang at top
