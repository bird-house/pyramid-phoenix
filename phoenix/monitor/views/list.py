...
class CaptionSchema(colander.MappingSchema):
    ...
    identifier = colander.SchemaNode(colander.String(), missing=None, widget=HiddenWidget())
...
class LabelsSchema(colander.MappingSchema):
    ...
    identifier = colander.SchemaNode(colander.String(), missing=None, widget=HiddenWidget())
    labels = colander.SchemaNode(colander.String(), title="Input labels as a comma-separated list", missing="dev")
    ...
    def process_caption_form(self, form):
        try:
            ...
            self.collection.update_one(
                {"identifier": appstruct["identifier"]}, {"$set": {"caption": appstruct["caption"]}}
            )
...
class JobsGrid(CustomGrid):
    ...
    def duration_td(self, col_num, i, item):
        return self.render_td(
            renderer="duration_td.mako", job_id=item.get("identifier"), duration=item.get("duration", "-")
        )

    def caption_td(self, col_num, i, item):
        return self.render_td(
            renderer="caption_td.mako", job_id=item.get("identifier"), caption=item.get("caption", "-")
        )
    ...
    def buttongroup_td(self, col_num, i, item):
        ...
        buttons.append(
            ActionButton(
                ...
                href=self.request.route_path("job_details", tab="outputs", job_id=item.get("identifier")),
            )
        )
