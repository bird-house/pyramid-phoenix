    ...
    OAUTH2 = {
        ...
        "keycloak": {  # keycloak
            "class_": create_keycloak_provider(
                url=request.registry.settings.get("keycloak.url"), realm=request.registry.settings.get("keycloak.realm")
            ),
...
def includeme(config):
    ...
    authn_policy = MyAuthenticationPolicy(settings.get("authomatic.secret"), callback=groupfinder, hashalg="sha512")
