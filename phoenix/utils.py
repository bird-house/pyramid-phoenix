    ...
    if "pinned_processes" in settings:
        for pinned in settings.get("pinned_processes"):
            try:
                ...
                url = request.route_path("processes_execute", _query=[("wps", service_name), ("process", identifier)])
                wps = WebProcessingService(url=request.route_url("owsproxy", service_name=service_name), verify=False)
