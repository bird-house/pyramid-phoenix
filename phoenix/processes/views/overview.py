...
class Overview(MyView):
    ...
    @view_config(
        route_name="processes", renderer="phoenix:processes/templates/processes/overview.pt", accept="text/html"
    )
    def view(self):
        return dict(title="Web Processing Services", items=self.wps_services(), processes=self.pinned_processes())
