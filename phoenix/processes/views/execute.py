...
class ExecuteProcess(MyView):
    def __init__(self, request):
        ...
        self.wps = WebProcessingService(url=self.service.url, verify=False)
    ...
    def breadcrumbs(self):
        ...
        breadcrumbs.append(
            dict(
                route_path=self.request.route_path("processes_list", _query=[("wps", self.service_id)]),
    ...
    def process_form(self, form):
        ...
        try:
            ...
            return dict(process=self.process, url=wps_describe_url(self.wps.url, self.processid), form=e.render())
        else:
    ...
    @view_config(
        route_name="processes_execute", renderer="phoenix:processes/templates/processes/execute.pt", accept="text/html"
    )
    def view(self):
        ...
        return dict(
            process=self.process, url=wps_describe_url(self.wps.url, self.processid), form=form.render(self.appstruct())
        )
