...
class Resources(object):
    ...
    @panel_config(name='myjobs_resources', renderer='phoenix:templates/panels/myjobs_default.pt')
    def panel(self):
        jobid = self.session.get('jobid')
        
        items = []
        from phoenix.models import process_outputs
        for oid,output in process_outputs(self.request, jobid, tab='resources').items():
